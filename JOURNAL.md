...
**Commit :** `1c2fc0a` — branche `mactar`

#### Supprimé
- `app/scraper_ft.py` — scraper France Travail (OAuth2 + API officielle)
- `app/find_emails.py` — recherche emails via Hunter.io
- `app/send_candidatures.py` — envoi batch depuis contacts.csv
- Clés `FT_CLIENT_ID`, `FT_CLIENT_SECRET`, `HUNTER_API_KEY` nettoyées de `config.py`

#### Ajouté
- `app/scrapers/mairies.py` — API DILA, mairies Île-de-France (75/77/91/93/94)
- `app/scrapers/education.py` — API Education nationale, lycées/collèges/CIO/GRETA
- `app/ai/lm_spontane.py` — génération LM sectorielle (même profil.yml, compétences admin en avant, BI/SQL/Python exclus)
- `app/models/cible_spontanee.py` — workflow `neuf → prêt → envoyé / erreur`
- `alembic/versions/0012_add_cible_spontanee.py` — migration table `cibles_spontanees`
- `config/cv_mairies.pdf`, `config/cv_education.pdf` — CV sectoriels (PJ auto)

#### Modifié
- `app/routers/spontane.py` — pipeline complet : scrape, generate-lm-batch, PATCH lm, send, send-one, pagination
- `frontend/src/api.ts` — nouvelles fonctions spontane avec `CiblesResponse` paginé et `spontanePatchLM`
- `frontend/src/components/SpontanePanel.tsx` — refonte : filtres secteur + département, pagination, `useEffect` reload, `pageRef` anti-stale closure
- `frontend/src/components/SpontaneContactModal.tsx` — flag `lmDirty` → PATCH avant envoi si LM éditée
- `frontend/src/App.tsx` — `PipelineBoard` masqué sur onglet Spontané

#### Bugs corrigés
- `PAGE_SIZE` déclaré entre deux imports (syntaxe invalide)
- Changement de filtre secteur ne rechargait pas la liste
- LM éditée dans le modal non sauvegardée avant envoi
- `page` stale dans les closures async

#### Résultats du premier scraping
| Secteur | Total | Avec email |
|---|---|---|
| Mairies (Île-de-France) | 806 | 755 |
| Éducation (Île-de-France) | 1 627 | 1 616 |
| **Total** | **2 433** | **2 371** |
